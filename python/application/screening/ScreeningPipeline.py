...
from ccpn.lib.Screening import writeBruker, createStdDifferenceSpectrum, matchedPosition
...
class ExcludeRegions(QtGui.QWidget):
  ...
  def __init__(self, parent=None, project=None, **kw):
    ...
    self.pulldownSolvents = PulldownList(self, grid=(0, 0), hAlign='r')
    ...
    self.scrollArea = ScrollArea(self, grid=(0, 1), gridSpan=(1,1))
  ...
  def pickPeaks(self):
    for sample in self.project.samples:
      for spectrum in sample.spectra:
       spectrum.peakLists[0].pickPeaks1dFiltered(ignoredRegions=None, noiseThreshold=0)
...
class MatchPeaks(QtGui.QWidget):
  ...
  def __init__(self, parent=None, project=None, **kw):
    ...
    self.minimumDistanceLabel = Label(self, text='Minimum Distance', hAlign='l', grid=(0, 0))
    self.minimumDistanceLine = LineEdit(self, text=self.minimumDistanceValue,  hAlign='l', grid=(0, 1))
    ...
    for sample in self.project.samples:
      ...
      for components in componentList:
        for sampleComponent, peakPositions in components.items():
          match = matchedPosition(stdPosition, peakPositions, tolerance=float(self.minimumDistanceLine.text()), minimumMatches=self.minimumPeaksBox.value())
