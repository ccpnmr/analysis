...
class ShowScreeningHits(CcpnDock, Base):
  def __init__(self, project, **kw):
    ...
    CcpnDock.__init__(self, name='Hit_Table')
  ...
  def createHitSelectionGroup(self):
    ...
    self.hitSelectionGroup.setFixedWidth(220)
  ...
  def createSettingGroup(self):
    ...
    self.settingButtons = ButtonList(self, texts = ['Settings ','Export ',],
                                callbacks=[self.createViewSettingButton, self.createExportButton,], icons=[self.settingIcon, self.exportIcon],
  ...
  def createMoleculeDisplay(self):
    ''' Documentation '''
    ...
    self.compoundView.setMinimumSize(200,200)
    self.compoundView.setMaximumSize(500,200)
    if self.colourScheme == 'dark':
      self.compoundView.setStyleSheet(""" background-color:  #000021;""")
    else:
  ...
  def showMolecule(self):
    ...
    if self.smiles is not None:
      compound = importSmiles(self.smiles)
      variant = list(compound.variants)[0]
  ...
  def spectraToDisplay(self):
    ...
    if hasattr(currentObjPulldown, 'substanceName'):
      substance = self.project.getByPid('SU:'+currentObjPulldown.substanceName+'.H')
      spectraToDisplay[0].append(substance.referenceSpectra[0])
    else:
      spectraToDisplay[0].append(currentObjPulldown.substance.referenceSpectra[0])
