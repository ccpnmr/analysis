...
def resNameAnalysis(chemComps=None):
  """ Generate map of possible string resNames to tuple of (molType, ccpCode)
  tuples.
  All ccpCodes, code1Letter, code3Letter are used for ChemCompOverview list;
  for passed-in ChemComps also chemCompSysNames are used. '

  Custom function for name analysis
  ...
  for molType in molTypeOrder:
    ...
    if chemComps:
      ...
      for chemComp in chemComps:
        if molType == chemComp.molType:
          ...
          # Add ccpCode and main names
          tags = {ccpCode.upper(), chemComp.code3Letter.upper()}
          tag = chemComp.code1Letter
          if tag and chemComp.className == 'StdChemComp':
            tags.add(tag.upper())
          if None in tags:
          ...
          for namingSystem in chemComp.namingSystems:
            for sysName in namingSystem.chemCompSysNames:
              tags.add(sysName.sysName.upper())

          # put in mapping
          ccId = (molType, ccpCode)
          for tag in tags:
            prevId = tmpResult.get(tag,())
            if ccId not in prevId:
              tmpResult[tag] = prevId + (ccId,)
    ...
    for ccpCode,tt in sorted(data.items()):
      ...
      ccpCodeUpper = ccpCode.upper()
      if ccpCodeUpper != ccpCode and ccpCodeUpper in data:
        if tt == data[ccpCodeUpper]:
          # We have this one in both mixed-case and upper case. Skip
          caseDuplicates.append((ccpCode,molType))
          continue
      ...
      if cifCode:
        ...
        # if cifCode not in data and truCif not in data:
        #   # Skip cifCodes dealt with elsewhere
        tags.add(cifCode.upper())
      if ccpCode.upper() != cifCode:
        tags.add(ccpCode.upper())
      code1L = tt[0]
      if code1L:
        tags.add(code1L.upper())
      # if code1L and code1L not in tmpResult:
      #   tags.add(code1L)
    ...
    for tag, val in tmpResult.items():
      ...
      if oldval is None:
        ...
        # clash between types
        if molType == 'other' and len(val) == 1:
          tt = chemCompOverview[molType][val[0][1]]

          for mt, code in oldval:
            oldtt = chemCompOverview[mt][code]
            # print('@@@ %s' % (tt,))
            # print('@@@ %s' % (oldtt,))
            if tt == oldtt:
              otherDuplicates.append((molType, tag))
              break
          else:
            tmpResult[tag] = oldval + val
         ...
         #   # Ignore clashes with 'other. NB 'other' must be last in loop.
          result[tag] = oldval + val
  #
  return result, caseDuplicates, otherDuplicates



if __name__ == '__main__':
  data, caseDuplicates, otherDuplicates = resNameAnalysis()
  print ('### %s mixed-case / uppper-case duplicates:' % len(caseDuplicates))
  for tt in sorted(caseDuplicates):
    print (' - %s' % (tt,))

  print ('\n\n### %s x-other duplicates:' % len(otherDuplicates))
  for tt in sorted(otherDuplicates):
    print (' - %s' % (tt,))

  print ('\n\n### %s duplicates:' % len([tt for tt in data.items() if len(tt[1]) > 1]))
  for tag, ccIds in sorted(data.items()):
    if len(ccIds) > 1:
      print('> %s: %s' % (tag, ccIds))