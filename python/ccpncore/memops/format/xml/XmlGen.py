...
dataObjClassName = '%s.%s' % (implPackageName,
                             metaConstants.dataObjClassName)
  ...
  def initLeafPackage(self, package):
    ...
    if package is self.implPackage:
      ...
      for pp in self.modelPortal.leafPackagesByImport():
        ...
        # set top level map
        mapping['abstractTypes'] = {'.prefix':prefix,
                                                    '.qName':qName,
                                                    '.name':'abstractTypes'}
        mapping['exolinks'] = {'.prefix':prefix,
                                          '.qName':qName, 
                                          '.name':'exolinks'}
        ...
        mapping['globalRelease'] = Version.currentModelVersion
...
def getRoleMap(elem, inClass, globalMap):
  ...
  if elem.container is inClass:
    ...
    if result is not None:
      ...
      if hierarchy == metaConstants.parent_hierarchy:
        # parent link
        result = None
 
      elif hierarchy == metaConstants.child_hierarchy:
        # intra- or inter-package childlink
        result['type'] = 'child'
        result['eType'] = 'cplx'
        result['content'] = globalMap[vtPrefix]['abstractTypes']
        if (not inClass.isAbstract and inClass.parentRole is None
            and elem.name == metaConstants.packageLocatorRole):
        ...
        if result is not None:
          ...
          if otherRole:
            if ( otherRole.hicard == 1
                 ...
                 or otherRole.changeability != metaConstants.changeable
            ):
              copyOverride = False
          #
