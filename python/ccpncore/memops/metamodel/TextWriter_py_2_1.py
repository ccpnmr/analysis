...
class TextWriter_py_2_1:
  ...
  def clearOutDir(self, topDirName, forceClearOut=False):
    ...
    # remove files
    for name in files:
      # NBNB TODO cannot be done till  directories are reorganised
      #os.remove(name)
      pass
  ...
  def openFile(self, fileName):
    ...
    # TODO: if only have one fp what is point of having multiple indents?
  ...
  def writeToFile(self, ss):
    ...
    if self.DEBUG_OUTPUT:
      indent = ' ' * self.indent
      ss = ss.strip()
      if (ss and not ss.startswith(self.startComment) and not
          ss.startswith(self.startMultilineComment)):
        import inspect
        stack = inspect.stack()
        n = 1
        while True:
        ...
        for m in range(4):
          try:
            ...
            return
          self.fp.write('%s%s %s %d %s %s\n' % (indent, self.startComment, fileRoot, lineNo,
                                                function, self.endComment))
        self.fp.write('\n')
  ...
  def getVersionString(self, metaobj=None, scriptName=None, date=None):
    ...
    of the generating script, and the date. Name defaults to
    self.scriptName, if this is set.
    ...
    if metaobj:
      pkgLine = '  from data model element %s'  % metaobj.qualifiedName()
    else:
      pkgLine = ''
    ...
    lines.append('')
    lines.append('Autogenerated by %s on %s' % (scriptName, date))
  ...
  def getCreditsString(self, metaobj, programType):
    ...
    from ccpncore.memops.license import data
