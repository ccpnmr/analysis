...
class GuiTableGenerator(QtGui.QWidget):
  ...
  def __init__(self, parent, objectLists, callback, columns, **kw):
  ...
  def _getColumns(self, columns):
    ...
    if self.objectList.shortClassName == 'PL':
      ...
      for i in range(numDim):
        j = i + 1
        c = Column('Assign\nF%d' % j,
                   lambda pk, dim=i:self.parent()._getPeakAnnotation(pk, dim),
      ...
      for i in range(numDim):
        ...
        c = Column(text,
                 ...
                 lambda pk, dim=i, unit=unit:self.parent()._getPeakPosition(pk, dim, unit),
                 tipText=tipText)
        tableColumns.append(c)
