...
class ParassignModule(Dock):
  ...
  def __init__(self, parent=None, peakLists=None, name='Peak List', **kw):
    ...
    #self.initPanel()
    label = Label(self, 'Diamagentic Peak List:', grid=(0, 0))
    self.layout.addWidget(label, 0, 0)
    self.diaPeakListPulldown = PulldownList(self, grid=(0, 1),
                                          callback=self.changePeakList,)

    label = Label(self, ' Paramagnetic Peak List:', grid=(0, 2))
    self.paraPeakListPulldown = PulldownList(self, grid=(0, 3), callback=self.changePeakList)
                                        # callback=self._updateWhenIdle,)

    self.peakTable = ObjectTable(self, self._getColumns(2), [],
                                 callback=self.markPeakPair, grid=(3, 0),
                                 gridSpan=(5, 5))
  ...
  def markPeakPair(self, peak, row, col):
    if not peak:
      return
    else:
  ...
  def _getColumns(self, numDim):
    ...
    columns.append(Column('PCS', self._getPeakHeight,
                             tipText='Magnitude of spectrum intensity at peak center (interpolated), unless user edited'))
