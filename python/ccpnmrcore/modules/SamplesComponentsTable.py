...
class PeakListSampleComponent(QtGui.QWidget):
  ...
  def __init__(self, parent=None, project=None, callback=None):
    ...
    self.peakListPulldown = PulldownList(self, grid=(0, 3), hAlign='r')
    ...
    self.peakTable = GuiTableGenerator(self, objectLists=self.peakLists, callback=None, columns=columns,
                                       selector=self.peakListPulldown, tipTexts=tipTexts) # selector=self.peakListPulldown
    self.layout().addWidget(self.peakTable, 3, 0, 2, 3)
  ...
  def pulldownSample(self, selection):
    ...
    for sample in self.project.samples:
      if sample.pid  == selection:
          pLsamples = [self.peakLists for spectrum in sample.spectra for self.peakLists in spectrum.peakLists]
          self.peakListPulldown.setData(pLsamples)
