...
#define FLOOR(x) (((x) == (int)(x)) ? (int)(x) : ((x) >= 0 ? (int)(x) : (-1 - ((int)(-(x))))))
...
#define OPEN_FOR_READING(fp, name) (((fp) = fopen((name), READ)) == NULL)

#define OPEN_FOR_WRITING(fp, name) (((fp) = fopen((name), WRITE)) == NULL)

#define OPEN_FOR_BINARY_READING(fp, name) (((fp) = fopen((name), BINARY_READ)) == NULL)

#define OPEN_FOR_BINARY_WRITING(fp, name) (((fp) = fopen((name), BINARY_WRITE)) == NULL)

#define OPEN_FOR_BINARY_READ_UPDATE(fp, name) (((fp) = fopen((name), BINARY_READ_UPDATE)) == NULL)

#define OPEN_FOR_BINARY_WRITE_UPDATE(fp, name) (((fp) = fopen((name), BINARY_WRITE_UPDATE)) == NULL)
...
#define MALLOC(ptr, type, num)                                                                     \
    {                                                                                              \
        if (((ptr) = (type *)malloc((unsigned)((num) * sizeof(type)))) == NULL) return CCPN_ERROR; \
    }

#define MALLOC_NEW(ptr, type, num)                                                           \
    {                                                                                        \
        if (((ptr) = (type *)malloc((unsigned)((num) * sizeof(type)))) == NULL) return NULL; \
    }
...
#define MALLOC2(ptr, type, num1, num2)                           \
    {                                                            \
        ...
        for (I = 0; I < num1; I++) MALLOC((ptr)[I], type, num2); \
    }
...
#define MALLOC2_ZERO(ptr, type, num1, num2)                           \
    {                                                                 \
        int J;                                                        \
        MALLOC(ptr, type *, num1);                                    \
        for (J = 0; J < num1; J++) MALLOC_ZERO((ptr)[J], type, num2); \
    }
...
#define FREE2(ptr, type, num)                               \
    {                                                       \
        int I;                                              \
        if ((ptr) != (type **)NULL) {                       \
            for (I = 0; I < num; I++) FREE((ptr)[I], type); \
            FREE(ptr, type *);                              \
        }                                                   \
    }
...
#define REALLOC(ptr, type, num)                                                         \
    {                                                                                   \
        type *Ptr;                                                                      \
        if (((Ptr) = (type *)realloc((ptr), (unsigned)((num) * sizeof(type)))) == NULL) \
            return CCPN_ERROR;                                                          \
        else                                                                            \
            ptr = Ptr;                                                                  \
    }
