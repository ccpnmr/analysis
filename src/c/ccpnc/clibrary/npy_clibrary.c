...
struct Node *insertConnectedNode(struct Node **headRef, PyObject *offsetItem,
                                 long index, CcpnBool isConnected,
  ...
  if (offsetTest != Py_None) {
    relOffset =
        PyFloat_AsDouble(PyObject_GetAttrString(offsetItem, "relativeOffset"));
...
static void getIndexInList(struct Node *node, PyObject *item, long *index,
                           long *found) {
  if (node) {
...
static PyObject *getNmrResidueIndex(PyObject *self, PyObject *args) {
  ...
  apiNmrResidue =
      (PyObject *)PyObject_GetAttrString(nmrResidue, "_wrappedData");
  if (!apiNmrResidue)
  ...
  PyDictObject *apiNmrChainDict =
      PyObject_GetAttrString(apiNmrChain, "__dict__");
  PyObject *isConnectedObj =
      PyDict_GetItemString(apiNmrChainDict, "isConnected");
  PyObject *apiNmrChainSerialObj =
      PyDict_GetItemString(apiNmrChainDict, "serial");

  // get all the resonanceGroups in the local apiNmrChain
  apiNmrResidues = (PyListObject *)PyObject_GetAttrString(
      apiNmrChain, "mainResonanceGroups");
  ...
  apiNmrProject =
      (PyObject *)PyObject_GetAttrString(apiNmrResidue, "nmrProject");
  apiNmrProjectDict =
      (PyDictObject *)PyObject_GetAttrString(apiNmrProject, "__dict__");
  resonanceGroupDict = (PyDictObject *)PyDict_GetItemString(apiNmrProjectDict,
                                                            "resonanceGroups");
  ...
  for (ii = 0; ii < rgSize; ii++) {
    ...
    mainRes = (PyObject *)PyObject_GetAttrString(
        listRes, "mainResonanceGroup"); //  not pointing to the right place?
  ...
  for (ii = 0; ii < numRes; ii++) {
    ...
    thisNode = insertConnectedNode(&resonanceList, mainRes, ii, isConnected,
                                   CCPN_TRUE);
    ...
    for (jj = 0; jj < numOffsets; jj++)
      if (offSetMainResonances[jj] == mainRes)
        offsetNode =
            insertConnectedNode(&resonanceList, offsetResonanceGroups[jj],
                                thisNode->index, isConnected, CCPN_FALSE);
...
static char getNmrResidueIndex_doc[] =
    "Return the index of an nmrResidue in a chain";

static struct PyMethodDef Clibrary_type_methods[] = {
    {"testReturnList", (PyCFunction)testReturnList, METH_VARARGS,
     testReturnList_doc},
    {"getNmrResidueIndex", (PyCFunction)getNmrResidueIndex, METH_VARARGS,
     getNmrResidueIndex_doc},
    {NULL, NULL, 0, NULL}};
