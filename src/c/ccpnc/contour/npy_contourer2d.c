...
static void delete_contour_vertices(Contour_vertices vertices, int nlevels, int npoints1)
{
    ...
    for (i = 0; i < vertices->nblocks; i++)
        FREE(vertices->vertex_store[i], struct _Contour_vertex);
...
#define  NEW_VERTEX0(v, d1, d2, x, y) \
         if (!(v = new_vertex0(contour_vertices, level, d1, d2, x, y))) \
	     return CCPN_ERROR;

#define  NEW_VERTEX1(v, d1, d2, x, y) \
         if (!(v = new_vertex1(contour_vertices, level, d1, d2, x, y))) \
	     return CCPN_ERROR;
		...
		int x, int y)
{
    ...
    *p_v_col = v_col;
    ...
    if (more_levels)
        update_new_range(contour_vertices, x, y, contour_vertices->are_levels_increasing ? START_RANGE : NEITHER);
		...
		int x, int y)
{
    ...
    *p_v_col = v_col;
    ...
    if (more_levels)
        update_new_range(contour_vertices, x, y, contour_vertices->are_levels_increasing ? NEITHER : START_RANGE);
		...
		int x, int y)
{
    ...
    if (more_levels)
        update_new_range(contour_vertices, x, y, contour_vertices->are_levels_increasing ? END_RANGE : NEITHER);
		...
		int x, int y)
{
    ...
    if (more_levels)
        update_new_range(contour_vertices, x, y, contour_vertices->are_levels_increasing ? NEITHER : END_RANGE);
		...
		int x, int y)
{
    ...
    *p_v_col = v_new;
    ...
    if (more_levels)
        update_new_range(contour_vertices, x, y, contour_vertices->are_levels_increasing ? NEITHER : NEITHER);
			...
			PyArrayObject *data, CcpnBool more_levels)
{
    ...
    for (r = 0; r < nrows_old; r++)
    {
        ...
        for (c = 0; c < ncol_ranges_old[r]; c++)
        {
            ...
            if ((b_old == 1) || (b_old == 2))
                NEW_VERTEX1(v_col, d_old0, d_new0, i0, i1);
...
static PyObject *contourer(PyObject *self, PyObject *args)
{
    ...
    if (PyArray_TYPE(levels_obj) != NPY_FLOAT)
        RETURN_OBJ_ERROR("levelsArray needs to be array of floats");

    if (PyArray_NDIM(levels_obj) != 1)
        RETURN_OBJ_ERROR("levelsArray needs to be NumPy array with ndim 1");
...
PyObject *PyInit_Contourer2d(void)
{
    ...
    /* check for errors */
    if (PyErr_Occurred())
        Py_FatalError("can't initialize module Contourer2d");
