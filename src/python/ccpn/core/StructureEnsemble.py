...
class EnsembleData(pd.DataFrame):
  ...
  def _containingObject(self, value):
    if (value is None
        or (isinstance(value, AbstractWrapperObject)
            and value.className in ('StructureEnsemble'), 'Model')
        ):
  ...
  def _idsSelector(self, expression: typing.Union[str, typing.Iterable[str]]) -> pd.Series:
    ...
    if isinstance(expression, str):
      expression = listFromString(expression)
    for i in expression:
      chain, seqId, name, atom = i.split(Pid.IDSEP)
  ...
  def _modelNumberSetter(self, value:pd.Series) -> None:
    ...
    containingOblect = self._containingObject
    if containingOblect is not None:
      if isinstance(containingOblect, StructureEnsemble):
        containingOblect.resetModels()
      else:
        # assert(containingObject.className == 'Model')
        containingOblect.structureEnsemble.resetModels()
