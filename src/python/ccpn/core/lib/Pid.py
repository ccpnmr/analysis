...
__dateModified__ = "$dateModified: 2024-12-07 15:24:50 +0000 (Sat, December 07, 2024) $"
...
# TODO-GWV: examine and remove this aspect
altCharacter = '^'
backupAltCharacter = '`'
...
class Pid(str):
    ...
    def getParentPid(self) -> Pid:
        """:return the Pid corresponding to the parent object.
        Does not assume the object is present; i.e. completely depends on the Pid parts and the class it represents
        """
        from ccpn.core._implementation.CoreModel import CoreModel
        if (klass := CoreModel._getClass(self.type)) is None:
            raise RuntimeError(f'{self} does not define a valid CoreClass object')
        if (parentKlass := klass._parentClass) is None:
            raise RuntimeError(f'{self} does not have a parent class')

        return Pid.new(parentKlass.shortClassName, *self.fields[ : -klass._numberOfIdFields])
