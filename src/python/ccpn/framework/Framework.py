...
from ccpn.util.Layout import createLayoutDirectory, getLayoutFile, _createLayoutFile, DefaultLayoutFile, saveLayoutToJson, updateSavedLayout
...
class Framework:
  ...
  def __init__(self, applicationName, applicationVersion, args=Arguments()):
    ...
    self.savedLayout  = None  # intialised by self._getUserLayout
  ...
  def _getUserLayout(self, userPath=None):
    """defines the application.savedLayout dictionary.
    ...
    if userPath:
      ...
      if savedLayoutPath:
        ...
        _createLayoutFile(self.project)
        self._getUserLayout()

    return self.savedLayout

  def saveLayout(self):
    updateSavedLayout(self.ui.mainWindow)

  def saveLayoutAs(self):
    fp = self.getSaveLayoutPath()
    try:
      saveLayoutToJson(self.ui.mainWindow, jsonFilePath=fp)
    except Exception as e:
      getLogger().warning('Impossible to save layout. %s' %e)

  def _getAutogeneratedLayoutFile(self):
    if self.project:
      layoutFile = getLayoutFile(self.project.path)
      return layoutFile
  ...
  def initGraphics(self):
    ...
    if restoreLayout:
      ...
      if layout:
        ...
        _initialiseCcpnModulesLayout(self._mainWindow, layout)
...
#      ("Load Spectrum...", lambda: self.loadData(text='Load Spectrum'), [('shortcut', 'ls')]),
      ...
      ("Layout", (("Save", self.saveLayout, [('enabled', True)]),
                 ...
                 ("Restore last", None, [('enabled', False)]),
                 ("Restore from file...", None, [('enabled', False)]),
                 (),
  ...
  def _saveProject(self, newPath=None, createFallback=True, overwriteExisting=True) -> bool:
    ...
    if not successful:
      ...
      self.ui.mainWindow._fillRecentProjectsMenu()
      saveLayoutToJson(self.ui.mainWindow)
