...
class UpdatePopup(CcpnDialogMainWidget):
    ...
    def resetFromServer(self):
        ...
        count = 0
        version = '-'
        # GWV: runProcess calls Popen, which does not accept Path objects but expects string,
        # as it checks downstream for cmd[0].endswith( '\'')
        _update = ccpnBatchPath / 'update.bat' if isWindowsOS() else \
                  ccpnBinPath / 'update'
        cmd = [_update.asString(), '--count', '--version']
