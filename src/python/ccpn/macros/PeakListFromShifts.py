...
Usage:
    ...
    - replace axesCodesMap accordingly
...
Warnings = "Warning: This macro is still in its test phase. It might not work or work partially. Always inspect the results!"
...
# What this macro needs to do:
# User needs to specify a chemical shift list (CSL)
# User specifies an existing spectrum
#   macro identifies axis codes and asks for atom types for each axis
#   (at some stage use Experiment types?)
...
# (Alternatively user specifies a new type of spectrum ...)
...
def _simulatedSpectrumFromCSL(csl, axesCodesMap):
    ...
    for nmrResidue in nmrResiduesOD:
        ...
        if len(i) != len(
                nmrAtomNames):  # if not all the NmrAtoms are found for a specific CSL cannot make the peak, incomplete assignment/shifts
...
class BMRBcslToV3(CcpnDialog):
    ...
    def __init__(self, parent=None, axesCodesMap=None, bmrbFilePath=None, directory=None,
        ...
        row += 1
        self.buttonList = ButtonList(self, ['Info', 'Cancel', 'Create'], [self._showInfo, self.reject, self._okButton],
                                     grid=(row, 1))
