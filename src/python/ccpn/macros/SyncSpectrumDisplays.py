...
class SyncSpectrumDisplaysTable(CustomDataFrameTable):
    ...
    """
    TableDelegateClass = PulldownDelegateModel
...
class SpectrumDisplaysSyncEditorModule(CcpnModule):
    ...
    def __init__(self, mainWindow, name='SpectrumDisplay Sync Editor (Alpha)'):
        ...
        ## Add core notifiers
        if self.project:
            self._spectrumDisplayNotifier = Notifier(self.project, Notifier.DELETE, 'SpectrumDisplay',
                                                     callback=self._onSpectrumDisplayDeleted, setterObject=self)
