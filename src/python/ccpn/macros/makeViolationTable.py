...
# check the first 2 restraintTables
resLists = project.restraintTables
print(f' restraintTables   {resLists}')
...
counts = [np.array([sum([
    len(contribs[res]) for res in pk.restraints if res and res.restraintTable == rList
    ])
    for pk in pks])
...
for lCount, rl in enumerate(resLists):
    ...
    for pk, cc, maxcc in zip(pks, counts[lCount], maxCount):
        _res = [(res.pid, _atom) for res in pk.restraints if res.restraintTable == rl
    ...
    dfs[rl] = pd.concat([allPkPids,
                         pd.DataFrame(ll, columns=[f'RestraintId_{lCount + 1}',
...
#                                              for res in pk.restraints if res.restraintTable == rl], 
...
for ii, resList in enumerate(resLists):
    if resList in violationResults:
        ...
        if (f'RestraintId_{ii + 1}' in _left.columns and f'Atoms_{ii + 1}' in _left.columns) and \
                (f'RestraintId_{ii + 1}' in _right.columns and f'Atoms_{ii + 1}' in _right.columns):
            print(f'     generating _out[{resList}] for ({ii + 1})')
            _out[resList] = pd.merge(_left, _right, on=[f'RestraintId_{ii + 1}', f'Atoms_{ii + 1}'], how='left').drop(columns=['PeakPid']).fillna(0.0)
