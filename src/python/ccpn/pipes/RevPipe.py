...
class RevGuiPipe(GuiPipe):
    preferredPipe = True
    pipeName = PipeName

    def __init__(self, name=pipeName, parent=None, project=None, **kw):
        super(RevGuiPipe, self)
        ...
        if self.project is not None:
            if spectra:
                for spectrum in spectra:
                    if spectrum:
                        data = None
                        # spectrum.intensities = ng.proc_base.rev(spectrum.intensities)
                getLogger().info('Rev Demo completed')

                return spectra
            else:
                getLogger().warning('Rev failed. Returned original spectra')
                return spectra
