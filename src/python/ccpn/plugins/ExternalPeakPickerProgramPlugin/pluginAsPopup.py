...
import pandas as pd
from ccpn.api import PluginBase, PluginGUIModule, aPath, undo, PluginBasePopup, showWarning, getLogger
...
class DemoGuiPopup(PluginBasePopup):
    """A class to create the GUI element of the plugin as a Popup """
    FIXEDWIDTH = True
    FIXEDHEIGHT = False
    ...
    def _runCallback(self, *args, **kwargs):
        ...
        Disables the button immediately after it is pressed,
        then delegates execution to the superclass implementation.
        """
...
class MyPickerModule(PluginBase):
    ...
    def run(self, **kwargs: Any):
        ...
        _fileWatcher = self.startFileWatcher([workDirPath],
                              callbackFunc=partial(self._onFileChanged, peakList, outputPath),
                              includeSuffixes={'.csv'})
