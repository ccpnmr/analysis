...
class StructureRefinementGuiPlugin(PluginModule):
    ...
    def __init__(self, mainWindow=None, plugin=None, application=None, **kwds):
        ...
        self.pluginPath = os.path.join(self.project.path,StructureRefinementGuiPlugin.className)
        ...
        self.guiDict  = OD([('General',OD()), ('Ensembles',OD()), ('Restraints', OD()), ('Engines', OD())])
        ...
        # Set ensembles, use checkboxes with own labels
        grid = _addVerticalSpacer(self.scrollAreaLayout,grid)
        grid = _addRow(grid)

        widget = Label(self.scrollAreaLayout,text='Ensembles', grid=grid)
        _setWidgetProperties(widget,_setWidth(columnWidths,grid))
        for ensemble in sorted(self.project.structureEnsembles):
            grid = _addColumn(grid)
            # Check whether to use the name, id or serial, docs are confusing here
            widget = CheckBox(self.scrollAreaLayout, text=ensemble.id, checked=True, grid=grid, tipText=help['Ensembles'])
            _setWidgetProperties(widget,_setWidth(columnWidths,grid))
            self.guiDict['Ensembles'][ensemble.id] = OD([('Active', widget)])
            self.settings['Ensembles'][ensemble.id] = OD([('Active', self._getValue(widget))])
        ...
        # Set mode for target distance calibration
        # Target distances are taken directly from the restraints, if there is a target distance that is less than the upper limit
        # If target distances are absent, or equal to the upper limits, then the average effective distance after fbhw solvent refinement
        # is used
        #
    ...
    def _runDataCheck(self):
        ...
        if ensembles == 0:
            inputWarning += 'No structure ensembles selected\n'
        if restraints == 0:
            inputWarning += 'No restraints selected\n'
        if engines == 0:
            inputWarning += 'No structure refinement software selected\n'
    ...
    def _writeSettings(self):
        ...
        if setupComplete == True:
            ...
            noteTitle = '{0} - Structure Refinement'.format(self.settings['General']['Run name'])
    ...
    def _createPidList(self):
        ...
        for ensembleId in self.settings['Ensembles']:
            if self.settings['Ensembles'][ensembleId]['Active'] == True:
                self.exportPidList.append('SE:' + ensembleId)
    ...
    def _addCalculationBlocks(self):
        ...
        sf_category = 'ccpn_yaria_structure_refinement_run'
    ...
    def _runCalculation(self):
        ...
        # """ Insert here the script for running StructureRefinement """
...
class StructureRefinementPlugin(Plugin):
    PLUGINNAME = 'YARIA structure refinement'
    guiModule = StructureRefinementGuiPlugin
    CCPNPLUGIN = True
