...
class XplorGuiPlugin(PluginModule):
  ...
  # def _checkXplorPath(self):
  #   self.XplorPath = self.userPluginPath+'Xplor/'
  #   if not os.path.exists(self.XplorPath):
  #     os.makedirs(self.XplorPath)

  def _run(self):
    pids = self.treeView.getSelectedObjectsPids()
    try:
      self.project.exportNef(str(self.XplorPath+self.runNameLineEdit.get()), pidList=pids)
  ...
  def _restrictChemicalShiftListSelection(self, item):
    '''Restrict selection of one chemical shift list per Calculation. Remove this when Xplor can handle multiple CSL '''
...
class XplorPlugin(Plugin):
  PLUGINNAME = 'Xplor'
  guiModule = XplorGuiPlugin

  def run(self, **kwargs):
    ''' Insert here the script for running Xplor '''
    print('Running Xplor', kwargs)




XplorPlugin.register() # Registers the pipe in the pluginList