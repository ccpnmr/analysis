...
class YasaraGuiPlugin(PluginModule):
  ...
  # def _checkYasaraPath(self):
  #   self.YasaraPath = self.userPluginPath+'Yasara/'
  #   if not os.path.exists(self.YasaraPath):
  #     os.makedirs(self.YasaraPath)

  def _run(self):
    pids = self.treeView.getSelectedObjectsPids()
    try:
      self.project.exportNef(str(self.YasaraPath+self.runNameLineEdit.get()), pidList=pids)
  ...
  def _restrictChemicalShiftListSelection(self, item):
    '''Restrict selection of one chemical shift list per Calculation. Remove this when Yasara can handle multiple CSL '''
...
class YasaraPlugin(Plugin):
  PLUGINNAME = 'Yasara'
  guiModule = YasaraGuiPlugin

  def run(self, **kwargs):
    ''' Insert here the script for running Yasara '''
    print('Running Yasara', kwargs)




YasaraPlugin.register() # Registers the pipe in the pluginList