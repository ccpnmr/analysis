...
__dateModified__ = "$dateModified: 2022-10-26 15:51:02 +0100 (Wed, October 26, 2022) $"
...
class ChemicalShiftsMapping(CcpnModule):
    ...
    def _customActionCallBack(self, selection, lastItem):

        try:
            if not (objs := list(lastItem[self._OBJECT])):
                return
        except Exception as es:
            getLogger().debug2(f'{self.__class__.__name__}._customActionCallBack: No selection\n{es}')
            return
