...
class MacroEditor(CcpnModule):
    ...
    def _openPythonConsoleModule(self):
        if self.mainWindow._pythonConsoleModule is None:  # No pythonConsole module detected, so create one.
        ...
        if self._restore:
            ...
            for variableName, value in widgetsState.items():
                try:
                    ...
                    getLogger().debug('Exception raised: Impossible to restore %s value for %s: %s' %
                                      (variableName, self.name(), e))
