...
__dateModified__ = "$dateModified: 2022-04-28 20:31:08 +0100 (Thu, April 28, 2022) $"
...
class PeakTableModule(CcpnModule):
    ...
    def _setWidgets(self):
        ...
        row += 1
        self._tableWidget = _NewPeakTableWidget(parent=_topWidget,
                                                mainWindow=self.mainWindow,
                                                moduleParent=self,
    ...
    def _selectionPulldownCallback(self, item):
        ...
        if self._table is not None:
            self._tableWidget.populateTable(selectedObjects=self.current.peaks)
        else:
...
class _NewPeakTableWidget(_SimplePandasTableViewProjectSpecific):
    ...
    def _updateTable(self):
        ...
        """
        if self._table and self._table.peaks:
            self.populateTable(selectedObjects=self.current.peaks)
        else:
