...
class _NewRestraintAITableWidget(_CoreMITableWidgetABC):
    ...
    def actionCallback(self, selection, lastItem):
        ...
        item = objs[0] if isinstance(objs, (tuple, list)) else objs

        if isinstance(item, Peak):
            self._actionPeak(item)
        # action for Collection?

    def _actionPeak(self, peak):
        # If the current strip contains the double-clicked peak will navigateToPositionInStrip
    ...
    def _raiseTableContextMenu(self, pos: QtCore.QPoint):
        action = super()._raiseTableContextMenu(pos)
        if action.text() == _RESTRAINTS_OPTION:
            # signal parent to enable refresh button :|
            # may have to count collections to see if was successful
            if self.resources and (gui := self.resources.guiFrame):
                # nasty :| may have to do something like QApplication.postEvent(self, custom_event)
    ...
    def buildTableDataFrame(self) -> pd.DataFrame:
        ...
        if cSetLists:
            ...
            # peaks and collections are now the indexed items in the table
            pkRestraints = {}
            pks = set()
            for resList in allResLists:
                for res in resList.restraints:
                    # add the peaks to the table-object list
                    for pk in res.peaks:
                        pks.add(pk)
