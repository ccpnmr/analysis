...
class ValidatePathsPopup(CcpnDialog):
    """
    ...
    def __init__(self, parent=None, mainWindow=None, spectra=None,
        ...
        self.insideUrlLabel = Label(self, "$INSIDE", grid=(row, 0), )
        ...
        self.alonsideUrlLabel = Label(self, "$ALONGSIDE", grid=(row, 0), )
        ...
        self.dataUrlLabel = Label(self, "User Data Path ($DATA)", grid=(row, 0), )
        ...
        # self.dataUrlButton = Button(parent, grid=(row, 2), callback=self._getDataPath, icon='icons/directory', hPolicy='fixed')
        row += 1
        ...
        allUrls = [dataUrl for store in self.project._wrappedData.memopsRoot.sortedDataLocationStores() for dataUrl in store.sortedDataUrls()]

        for dataUrl in allUrls:

            urlLabel = Label(self.scrollAreaWidgetContents, text=dataUrl.name, grid=(scrollRow, 0))
            urlData = LineEdit(self.scrollAreaWidgetContents, textAlignment='left', grid=(scrollRow, 1))
            urlData.setValidator(FilePathValidator(parent=urlData, dataUrl=dataUrl))
            urlButton = Button(self.scrollAreaWidgetContents, grid=(scrollRow, 2), callback=partial(self._getDataUrlDialog, dataUrl),
                               icon='icons/applications-system')

            self.dataUrlData[dataUrl] = (urlData, None, urlLabel)
    ...
    def _getDataUrlDialog(self, dataUrl):
        ...
        if dataUrl and dataUrl in self.dataUrlData:
            ...
            if len(directory) > 0:
                ...
                # dataUrl.url = Implementation.Url(path=newUrl)
    ...
    def _setUrlData(self, dataUrl):
        ...
        #     urlData, urlButton, urlLabel = self.dataUrlData[dataUrl]
        #
        #     apiDataStore = dataUrl._apiDataSource.dataStore
        #     if not apiDataStore:
        #         urlData.setText('<None>')
        ...
        #             urlData.setText('$INSIDE/%s' % apiDataStore.path)
        #         elif dataUrlName == 'alongsideData':
        #             urlData.setText('$ALONGSIDE/%s' % apiDataStore.path)
        #         elif dataUrlName == 'remoteData':
        #             urlData.setText('$DATA/%s' % apiDataStore.path)
        #     else:
        #         urlData.setText(apiDataStore.fullPath)
        #
    ...
    #     for dataUrl in self.spectra:
    # 
    #         if dataUrl in self.dataUrlData and dataUrl.isValidPath:
    #             widgets = self.dataUrlData[dataUrl]
    #             for widg in widgets:
    ...
    #     for dataUrl in self.spectra:
    # 
    #         if dataUrl in self.dataUrlData and not dataUrl.isValidPath:
    #             widgets = self.dataUrlData[dataUrl]
    #             for widg in widgets:
    #                 widg.setVisible(visible)
