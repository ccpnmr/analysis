...
__dateModified__ = "$dateModified: 2024-03-19 14:42:18 +0000 (Tue, March 19, 2024) $"
...
class ZoomPopup(CcpnDialog):
    ...
    def __init__(self, parent=None, mainWindow=None, **kwds):
        ...
        if self.current.strip:
            ...
            self.buttonBox = ButtonList(self, grid=(5, 0), gridSpan=(1, 4), texts=['Cancel', 'Set Zoom'],
                                        callbacks=[self.reject, self._zoom])
        else:
            self.close()
    ...
    def _zoom(self):
        ...
        with undoBlockWithoutSideBar():
            ...
            if not self.current.strip:
                self.reject()
            for strip in self.current.strip.spectrumDisplay.strips:
                strip.zoom(positions[0], positions[1])

            GLSignals = GLNotifier(parent=None)
            GLSignals._emitAxisLockChanged(strip=self.current.strip,  lockValues=(0,))
        self.accept()
