...
class GuiPipe(Dock, GuiPipeDrop):
    ...
    def __init__(self, parent, name, project=None, widgetsParams=None, activateSignals=True, **kwds):
        """
        ...
        self.activateSignals = activateSignals
    ...
    def _setPipeSignal(self, active=True):
        if active:
            self.pipe._progressSignal.connect(self._updateProgress)
        else:
            try:
                self.pipe._progressSignal.disconnect(self._updateProgress)
            except Exception as err:
                getLogger().warning('Disabling Pipe signal. Nothing to disable', err)

    def _setRunningStyle(self):
        self.label.setStyleSheet(_getPipeBoxStyleSheet(_runningOrange))

    def _setStandbyStyle(self):
        self.label.setStyleSheet(_getPipeBoxStyleSheet(_standbyBlue))
        self.label.progressLabel.setText('Ready')

    def _setFailedStyle(self):
        self.label.setStyleSheet(_getPipeBoxStyleSheet(_failedRed))
        self.label.progressLabel.setText('Failed')

    def _setCompletedStyle(self):
        self.label.setStyleSheet(_getPipeBoxStyleSheet(_completedGreen))
        self.label.progressLabel.setText('Completed')

    def _setNonThreadStyle(self):
        self.label.setStyleSheet(_getPipeBoxStyleSheet(_defaultGreen))
        self.label.progressLabel.setText('')
