...
__dateModified__ = "$dateModified: 2024-10-18 10:00:54 +0100 (Fri, October 18, 2024) $"
...
class ViewBox(pg.ViewBox):
    ...
    def __init__(self, strip):
        ...
        self.strip = strip
        self.application = strip.spectrumDisplay.mainWindow.application
...
def _peaksVisibleInStrip(peaks, strip):
    ...
    for spectrumView in strip.spectrumViews:
        if spectrumView.spectrum.dimensionCount == 1:  # skip 1D peakLists
            continue
        indices = spectrumView.dimensionIndices[:2]
