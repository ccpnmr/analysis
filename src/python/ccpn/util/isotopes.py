...
class IsotopeRecord(CcpNmrJson):
    ...
    mass =             Float(allow_none=True, default_value=None).tag(info='The mass (in dalton)')
    ...
    def __init__(self, isotopeCode=None, elementNumber=None, mass=None,
        ...
        if mass is not None: self.massNumber=mass
...
# class IsotopeRecord2(object):
#     """Class to store isotope information
#     """
#     _attributes =  ('isotopeCode', 'elementNumber', 'massNumber', 'isRadioactive',
#                     'symbol', 'name', 'spin', 'gFactor','abundance', 'quadrupoleMoment'
#                    )
#
...
#         self.quadrupoleMoment=quadrupoleMoment
#
#     def update(self, fromDict):
#         "Update self using key, values in fromDict"
...
#
#     def toJson(self):
#         "Convert to a json string encoded as a dict"
#         return json.dumps(self.asDict())
#
#     def fromJson(self, string):
#         "Update self from a json encoded dict string"
#         theDict = json.loads(string)
#         self.update(theDict)
#
#     def __str__(self):
#         return '<IsotopeRecord %s>' % self.isotopeCode
#
#     def __repr__(self):
#         return 'IsotopeRecord(%s)' % ', '.join(['%s=%s'%(k,v) for k,v in self.asDict().items()])
