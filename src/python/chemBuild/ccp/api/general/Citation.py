...
class Citation(memops.api.Implementation.DataObject):
  ...
  def findAllMolecules(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      tobjList = list(root.__dict__.get('molecules').values())
      for tobj in tobjList:
        if (not (tobj.__dict__.get('isLoaded'))):
          tobj.load()
...
class CitationStore(memops.api.Implementation.TopObject):
  ...
  def _checkDelete(self, objsToBeDeleted, objsToBeChecked, linkCounter, topObjectsToCheck):
    ...
    citations = list(dataDict.get('citations').values())
  ...
  def checkAllValid(self, complete=False):
    ...
    values = list(dataDict.get('citations').values())
    for value in values:
  ...
  def findAllCitations(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      currentValues = list(dataDict.get('citations').values())
      result = set(currentValues)
  ...
  def getCitations(self):
    ...
    tempVar = list(dataDict.get('citations').values())
    result = frozenset(tempVar)
    return result
