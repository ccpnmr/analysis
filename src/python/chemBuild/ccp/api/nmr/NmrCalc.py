...
class Run(memops.api.Implementation.DataObject):
  ...
  def _checkDelete(self, objsToBeDeleted, objsToBeChecked, linkCounter, topObjectsToCheck):
    ...
    data_s = list(dataDict.get('data').values())
    for data in data_s:
      if (not (data in objsToBeDeleted)):
        objsToBeDeleted.add(data)
        objsToBeChecked.append(data)

    runParameters = list(dataDict.get('runParameters').values())
  ...
  def checkAllValid(self, complete=False):
    ...
    dataDict = self.__dict__
    self.checkValid(complete)
    values = list(dataDict.get('data').values())
    for value in values:
      value.checkAllValid(complete)

    values = list(dataDict.get('runParameters').values())
    for value in values:
      value.checkAllValid(complete)
  ...
  def findAllData(self, **conditions):
    ...
    nConditions = len(conditions)
    if (nConditions == 0):
      currentValues = list(dataDict.get('data').values())
      result = set(currentValues)
  ...
  def findAllRunParameters(self, **conditions):
    ...
    nConditions = len(conditions)
    if (nConditions == 0):
      currentValues = list(dataDict.get('runParameters').values())
      result = set(currentValues)
  ...
  def getData(self):
    ...
    """
    dataDict = self.__dict__
    tempVar = list(dataDict.get('data').values())
    result = frozenset(tempVar)
    return result
  ...
  def getRunParameters(self):
    ...
    """
    dataDict = self.__dict__
    tempVar = list(dataDict.get('runParameters').values())
    result = frozenset(tempVar)
    return result
