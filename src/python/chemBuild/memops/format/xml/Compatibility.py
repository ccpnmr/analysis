...
class DummyClass(object):
  ...
  def __init__(self, parent, **attrlinks):
    ...
    # set attributes
    for key,value in list(attrlinks.items()):
      ...
      for residue in chain.residues:
        ...
        for atom in residue.atoms:
          ...
          if chemAtom is None:
            ...
            for ccv in list(chemCompVars.keys()):
              if ccv not in ccvs:
    ...
    for chain in molSystem.chains:
      for residue in chain.residues:
        ...
        if len(chemCompVars) == 1:
          descriptor = list(chemCompVars.keys())[0].descriptor
          
        else:
          # choose Var with fewest atoms.
          ll = [(y,x) for x,y in list(chemCompVars.items())]
          ll.sort()
...
for ll in list(mappingMakers.values()):
  ll.sort()
  
for ll in list(dataConverters.values()):
  ll.sort()
...
def special_conv_1_0_204(project):
  ...
  if molDict:
    ...
    # fix Molecules
    for molecule,corr in list(molDict.items()):
      for molResidue in molecule.molResidues:
