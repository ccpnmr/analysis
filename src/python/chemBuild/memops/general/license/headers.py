...
def doSetLicenses(curDir, licenseDir, infoModule, level, warnSkippedFiles, mode):
  ...
  if infoModule is None:
    ...
    infoRange = list(range(len(infoModule.licenseInfo)))
...
def addHeader(directory, fName, paramIndex, infoModule, mode):
      ...
      for expr in formatDict['ignoreStartLines']:
        ...
        if startIndex == -1:
          ...
          while ind == -1:
            ii = ii + 1
            if ii >= nlines:
              raise Exception(" ERROR, file %s contains unclosed header comment")
            line = lines[ii]
  ...
  if mode == 'act':
    ...
    raise Exception("ERROR, illegal mode parameter %s" % repr(mode))

def getFormat(fName):
  """ find corrent format
  """
  for tag,dict in list(formats.items()):
...
def showTestInfo():
  ...
  ll0 = []
  for infoMod,dict in list(testInfo.items()):
...
def getHeader(**parameters):
  ...
  if programFunction is None:
    ...
    raise Exception("Error in getHeader: programFunction must be a single line")
  
  if not (parameters.get('stdContact') or parameters.get('extraContact')):
    raise Exception("Error in getHeader: either stdContact or extraContact must be given")
