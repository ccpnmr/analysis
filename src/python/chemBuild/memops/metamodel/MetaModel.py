...
def compareModels(model1, model2, elementPairings=None, ignoreImplicit=True):
  ...
  # compare models
  for guid,ee in list(dict1.items()):
...
def compareElements(ee, ee2, ignoreImplicit=True, language='python'):
  ...
  if ee2.__class__ is clazz:
    ...
    for tag,pData in list(clazz.parameterData.items()):
...
def makeObjDict(rootElement, ignoreImplicit=False, crucialOnly=False):
  ...
  for ee in ll:
    ...
    if not (ignoreImplicit and ee.isImplicit):
      result[ee.guid] = ee
      ll.extend(list(ee._MetaModelElement__elementDict.values()))
      if isinstance(ee,ConstrainedElement):
        ll.extend(list(ee._ConstrainedElement__constraints.values()))
  #
  return result
...
def finaliseMetaClass(clazz):
  ...
  allowedParTags = {
   ...
   'enumeration':tuple,
   'isFixed':None,
   'setterFunc':str,#bytes,
   'getterFunc':str,#bytes,
   'namelist':str,#bytes,
   'isLink':int,
  }
  ...
  for pName, pData in list(clazz.parameterData.items()):
    
    # check parameter tags and value types
    for tag,val in list(pData.items()):
...
class MetaModelElement:
  ...
  def __init__(self, **params):
    ...
    if nn:
      ll2 = [x for x in list(params.keys()) if x not in ll]
...
%s""" % (self,nn,ll2))
  ...
  def _getCloningDict(self):
    ...
    """
    result = {}
    for pName,pData in list(self.__class__.parameterData.items()):
  ...
  def checkValid(self, complete=False):
    ...
    for tag, pData in list(self.parameterData.items()):
        ...
        for key,val in list(value.items()):
          if not key or not isinstance(key, bytes):
            raise MemopsError("%s: non-string or empty key %s in StringDict %s"
                              % (self, repr(key), tag))
          if not val or not isinstance(val, bytes):
      ...
      if self._tempData:
        raise MemopsError(
         "%s - _tempData dictionary is not emptied, keys are %s" % 
                          (self,list(self._tempData.keys())))
    
    
    # recursive contents check:
    for obj in list(self.__elementDict.values()):
...
class ConstrainedElement(MetaModelElement):
  ...
  def checkValid(self, complete=False):
    ...
    for constraint in list(self.__constraints.values()):
  ...
  def checkValid(self, complete=False):
    ...
    if name == ImpConstants.serial_attribute:
      ...
      serialValues = {'isAutomatic':True, 'changeability':ImpConstants.frozen,
                     'hicard':1, 'locard':1,}
      for key,val in list(serialValues.items()):
        x = getattr(self,key)
        if x != val:
    ...
    for supertype in allSupertypes[1:]:
      ...
      if superElem is not None:
        ...
        parameterData = self.parameterData
        for tag in list(parameterData.keys()):
          ...
          if val == superval:
            ...
            for tt,vv in list(superval.items()):
              if val.get(tt) != vv:
                raise MemopsError(
        ...
        for name in list(self._ConstrainedElement__constraints.keys()):
...
class ComplexDataType(AbstractDataType):
  ...
  def checkValid(self, complete=False):
    ...
    if constructorCode:
      ...
      for codeTag in list(constructorCode.keys()):
    ...
    for opType,opSubType,target in list(opCheckDict.keys()):
...
class MetaClass(ComplexDataType):
  ...
  def delete(self):
    ...
    for obj in list(self._MetaModelElement__elementDict.values()):
  ...
  def checkValid(self, complete=False):
    ...
    for tag in ('destructorCodeStubs','postDestructorCodeStubs'):
      ...
      if destructorCode:
        ...
        codeTags = ImpConstants.codeStubTags
        for codeTag in list(destructorCode.keys()):
...
class MetaDataType(AbstractDataType):
  ...
  def isValid(self,value):
    ...
    try:
      for constraint in list(self._ConstrainedElement__constraints.values()):
...
class MetaOperation(HasParameters):
  ...
  def checkValid(self, complete=False):
    ...
    # check code
    codeTags = ImpConstants.codeStubTags
    for codeTag in list(self._MetaModelElement__dataDict['codeStubs'].keys()):
...
class MetaConstraint(MetaModelElement):
  ...
  def checkValid(self, complete=False):
    ...
    codeTags = ImpConstants.codeStubTags
    for codeTag in list(codeStubs.keys()):
