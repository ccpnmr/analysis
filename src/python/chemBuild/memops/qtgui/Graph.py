...
BOX_COLOR_FG = QtGui.QColor( 128, 128, 128, 128)
...
# # # # #  T O  D O  # # # # #
#
# Menu set symbol, type, symSize & color per data set
...
class AxisItem(QtWidgets.QGraphicsItem):
  ...
  def __init__(self, parent, axis=None, isVertical=False, secondAxis=False):
    ...
    self.parent.scene().addItem(self)
    self.label = MovableLabelItem(parent, angle)
  ...
  def setAxis(self, axis):
    ...
    if axis.ticks:
      ...
      for i in range(int(round(dataWidth/dataStep))+2):
    ...
    if self.isVertical:
      ...
      startPoint = QPointF(plotMin, box[3])
      endPoint = QPointF(plotMax, box[3])
  ...
  def paint(self, painter, option, widget):
    ...
    painter.setPen(Qt.black) # Could set width
...
class CoordsItem(QtWidgets.QGraphicsItem):
  ...
  def paint(self, painter, option, widget):
    ...
    if self.hover:
      painter.setPen(BOX_COLOR_BG.darker())
      painter.setBrush(BOX_COLOR_BG.darker())
    
    else:
      painter.setPen(BOX_COLOR_BG)
      painter.setBrush(BOX_COLOR_BG)
...
class LegendItem(QtWidgets.QGraphicsItem):
  ...
  def set(self, dataSets):
    ...
    if dataSets:
      ...
      colors, texts, symbols, grads = list(zip(*data))
  ...
  def paint(self, painter, option, widget):
    ...
    if self.drawData:
      ...
      if self.hover:
        painter.setPen(BOX_COLOR_FG.darker())
        painter.setBrush(BOX_COLOR_BG.darker())
      
      else:
        painter.setPen(BOX_COLOR_FG)
        painter.setBrush(BOX_COLOR_BG)
...
class Graph(QtWidgets.QGraphicsView, Base):
  ...
  def __init__(self, parent, axes, dataSets=None, title=None, size=(400, 300), zoom=1.0,
    ...
    self.titleItem.set(0.5 * (x0+x1), y0-25, self.title)
    self.refresh()
  ...
  def updateData(self, dataSets, title=None, axes=None):
    ...
    if axes:
      ...
      self.xAxis.valRange = (INF, -INF)
      self.yAxis.valRange = (INF, -INF)
      
      if self.yAxis2:
        self.yAxis2.valRange = (INF, -INF)
  ...
  def drawBackground(self, painter, viewRect):
    ...
    painter.fillRect(sceneRect, QtGui.QBrush(Qt.white))
   
    #painter.setPen(QtGui.QColor(192, 0, 192, 255))
    #painter.drawRect(sceneRect) # 
    
    pr0, pr1, pr2, pr3 = self.getPlotRegion()
       
    painter.setPen(QtGui.QColor(192, 192, 192, 255))
  ...
  def getPlotRegion(self):
    ...
    return (x0+xpad, y0+ypad, x0+xpad+w, y0+ypad+h)     
...
if __name__ == '__main__':
  ...
  graph.addDataSet( GraphDataSet(points, 'x\u207b\xb9', '#0000C0',
