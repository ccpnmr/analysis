...
from .ButtonList import ButtonList, UtilityButtonList
...
from .TableModel import ObjectTableModel, HORIZONTAL
...
class Column:
  ...
  def __init__(self, heading, getValue, getEditValue=None, setEditValue=None,
               ...
               editDecimals=None, editStep=None, alignment=Qt.AlignLeft,
               orderFunc=None):
...
class ObjectTableFilter(BasePopup):
  ...
  def __init__(self, table):
    ...
    objects = list(range(len(columns)))
...
class ObjectTableExport(BasePopup):
  ...
  def saveFile(self):
    ...
    from .FileSelect import selectSaveFile, FileType
    from .MessageDialog import showWarning, showError
...
if __name__ == '__main__':
  ...
  from .Colors import ColorPulldown, DEFAULT_COLORS
  ...
  newObjs = (TestObj(1, 'Partridge in a pear tree', '\u03B1'),
             TestObj(2, 'Turtle doves', '\u03B2'),
             TestObj(3, 'French hens','\u03B3'),
             TestObj(4, 'Calling birds', '\u03B4'),
             TestObj(5, 'Golden rigs', '\u03B5'),
             TestObj(6, 'Geese a laying', '\u03B6'))
